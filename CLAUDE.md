...
- **Version lockstep**: bump `Cargo.toml` + `Cargo.lock`, `clients/typescript/package.json` (and its five `optionalDependencies`), every `clients/typescript/packages/*/package.json`, `clients/ruby/lib/llmshim/version.rb`, and the OpenAPI version together. Python derives from `Cargo.toml`. Before any publisher or artifact builder starts, the release workflow requires a documented `vMAJOR.MINOR.PATCH` tag that matches the root crate/lock and every lockstep value; it also verifies catalog manifest/dependency/lock consistency while preserving the catalog's independent version.

...
impl Limits {
    pub(crate) const UNARY: Self = Self {
        max_depth: 128,
        max_nodes: 65_536,
        max_owned_bytes: 40 * 1024 * 1024,
    };
    pub(crate) const SSE: Self = Self {
        max_depth: 128,
        max_nodes: 16_384,
        max_owned_bytes: 16 * 1024 * 1024,
    };
    pub(crate) const INBOUND: Self = Self {
        max_depth: 128,
        max_nodes: 32_768,
...
pub(crate) fn parse_slice(input: &[u8], limits: Limits) -> Result<Value, ParseError> {
    let mut deserializer = serde_json::Deserializer::from_slice(input);
    parse(&mut deserializer, limits)
}

pub(crate) fn parse_str(input: &str, limits: Limits) -> Result<Value, ParseError> {
...
impl<'de> Visitor<'de> for ValueVisitor<'_> {
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut map: A) -> Result<Value, A::Error> {
        ...
        while let Some(key) = map.next_key::<String>()? {
            self.budget.charge_key(key.len())?;

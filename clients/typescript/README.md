...
On the first `chat`/`stream`/`models`/`health` call on a `Client` constructed without `baseUrl`, the client finds that bundled binary (falling back to `PATH`, e.g. a `cargo install llmshim`) and starts it on a child-selected loopback port. The child publishes a one-time TLS certificate and authentication token over its private startup pipe. The client pins that certificate and authenticates every managed request, so another local listener cannot impersonate a child that exits.
